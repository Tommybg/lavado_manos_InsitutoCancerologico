...
interface CameraFeedProps {
  ...
  currentStep: number;
  onStepError: (step: number, message: string) => void;
}

const CameraFeed: React.FC<CameraFeedProps> = ({ isActive, onStart, showSuccess, onStepComplete, currentStep, onStepError }) => {
  const webcamRef = useRef<Webcam>(null);
  const [error, setError] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(true);
  const [cameraReady, setCameraReady] = useState(false);
  const [stepTimer, setStepTimer] = useState(0);
  const [detectedStep, setDetectedStep] = useState<number | null>(null);
  ...
  const processFrame = async () => {
    ...
    try {
      const response = await fetch('http://localhost:5000/detect-step', {
        method: 'POST',
        headers: {
      ...
      if (data.success) {
        const detected = data.detected_step;
        setDetectedStep(detected);
        
        if (detected === currentStep) {
          setStepTimer(prev => prev + 1);
          if (stepTimer >= 7) { // 7 seconds threshold
            onStepComplete(currentStep);
            setStepTimer(0);
          }
        } else {
          setStepTimer(0);
          onStepError(currentStep, `Please perform step ${currentStep}`);
        }
      }
    } catch (error) {
    ...
    <div className="camera-container relative w-full h-[180px] md:h-[350px] rounded-xl flex flex-col items-center justify-center transition-all duration-300 bg-gray-100 overflow-hidden">
      ...
      {error ? (
        ...
        <>
          ...
          <div className="absolute bottom-4 right-4 bg-green-500 text-white px-3 py-1 rounded">
            Progress: {stepTimer}s / 7s
          </div>
        </>
      )}
    </div>
